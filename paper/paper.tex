...
\section{Discussion}
The combined evidence supports a sparse-key view of entity knowledge in Qwen2.5-7B base. Localization is strong, concentrated in early blocks, and robust to surface-form variation. Controlled injection recovers answer-token probability in QA-style prompts relative to no-injection and wrong-cell controls, and negative ablation can selectively degrade entity knowledge relative to a control entity without collapsing model behavior. In exploratory cross-model checks (Appendix~\ref{app:cross_model}), the same protocol remains directional in Qwen2.5-7B-Instruct and OLMo-7B, is weaker in Mistral-7B, and is less calibrated on OpenLLaMA-7B under our normalization, suggesting both transfer and architecture-specific sensitivity.
...
\section{Cross-Model Quick Check}
\label{app:cross_model}
We run a quick transfer check of the Finding~2 protocol on additional open $\sim$7B models. To keep runtime manageable, we use a reduced setting: PopQA curated list subset ($N{=}40$, $K{=}1$), fresh localization per model (no reused coordinates), mean-entity initialization, top-$k$ injection ($k{=}5$), and per-entity $\alpha$ sweep over $\{1,2,4,8,16,32,64,128,200\}$.
...
\toprule
\textbf{Model} & \textbf{$n$ used} & \textbf{No inj.} & \textbf{Correct} & \textbf{Wrong} & \textbf{$k_{\mathrm{succ}}/n$} & \textbf{$k_{\mathrm{topk}}$} \\
\midrule
Qwen2.5-7B-Instruct & 40 & 0.154 & 0.781 & 0.309 & 27/40 & 2 \\
Mistral-7B-v0.3 & 40 & 0.022 & 0.033 & 0.026 & 5/40 & 0 \\
OLMo-7B-0724 & 37 & 0.073 & 0.661 & 0.117 & 23/37 & 6 \\
OpenLLaMA-7B & 33 & 3.533 & 5.018 & 4.903 & 12/33 & 2 \\
\bottomrule
\end{tabular}
\caption{Quick cross-model transfer check for Finding~2. Columns show mean relative answer probability ($\mathrm{RelProb}$) under no injection, correct-cell injection, and wrong-cell injection; $k_{\mathrm{succ}}/n$ uses the same success criterion as Finding~2, and $k_{\mathrm{topk}}$ counts entities that pass only with top-$k$. OpenLLaMA ratios are high because the denominator ($p_{\mathrm{ans}}$ under entity-present prompts) is often very small in this setup; directional separation is more informative than absolute scale for that row.}
\label{tab:cross_model_quick}
\end{table}
...
\toprule
\textbf{Attempted model} & \textbf{Outcome} \\
\midrule
meta-llama/Llama-3.1-8B-Instruct & Gated repository (HF access/authentication required) \\
google/gemma-2-9b-it & Gated repository (HF access/authentication required) \\
tiiuae/falcon-7b-instruct & Unsupported activation path in current script \\
\bottomrule
\end{tabular}
\caption{Attempted models without usable runs in this environment.}
\label{tab:cross_model_blocked}
\end{table}
