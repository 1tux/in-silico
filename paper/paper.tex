...
\begin{abstract}
Large language models exhibit strong entity-centric behavior, but the internal features that identify entities across surface forms remain poorly characterized. We present a mechanistic study in Qwen2.5-7B base that localizes sparse entity-sensitive MLP features (entity cells), validates them with causal interventions, and tests whether modulation can remain selective. Across a curated PopQA-200 benchmark, we find stable entity cells concentrated in early layers and robust to typos, acronyms, and multilingual forms. In QA-style prompts, controlled activation injection on top of a mean-entity initialization increases answer-token probability for the matched entity relative to wrong-cell controls, while negative ablation induces entity-specific amnesia with minimal control-entity drift. Extending the same pipeline to six additional open $\sim$7B models shows limited transfer: sparse candidates are detectable, but strong causal and robustness signals are concentrated in the Qwen family (and partly OLMo). These results support a practical view of entity cells as canonicalization anchors and pointer-like control features for interpretable factual modulation.
\end{abstract}

\section{Introduction}
Large language models (LLMs) encode extensive factual behavior about entities, but the mechanism that links varied surface forms to the same latent identity is still unclear. A practical question is whether sparse internal features act as canonicalization anchors that route computation toward an entity-consistent state. This question matters for interpretability, model editing, and safety, because such anchors would provide lightweight intervention points without full retraining.

We focus on \emph{named entities} (people, organizations, places), because entity-centric behavior is both pervasive and structured: one entity links to many relations (birthplace, spouse, founders, location, etc.). Our core mechanistic question is:
\begin{quote}
\small \emph{Does a pretrained base LLM contain sparse MLP neurons that behave as stable entity-sensitive pointers across diverse prompts, and do these features causally modulate entity-specific factual recall?}
\end{quote}
...
\paragraph{Overview of Findings.} The four primary findings establish early-layer concentration, causal modulation under controlled injection, robustness to name variants, and entity-specific amnesia under single-cell ablation. Appendices provide prompt templates, algorithms, and the full PopQA-200 entity cell map (neuron coordinates).
...
\textbf{MLPs as associative/routing features.} Feed-forward blocks have been modeled as key--value memories \cite{geva2021kv}, and embedding-space analyses seek to characterize these associations \cite{dar2023analyzingtransformersembeddingspace}. Our results are also consistent with an alternative interpretation: sparse MLP features can act as entity-sensitive routing coordinates that help canonicalize surface forms into a shared latent identity state. We therefore treat localized cells as leverage points in a broader distributed circuit, not isolated memory containers.
...
\end{equation}
with $v$ set to an entity-specific value estimated from the entity-present prompts in Finding 2. In controlled injection, this overwrite is applied on top of a mean-entity initialization, so the intervention probes directional movement on an existing entity manifold rather than de novo reconstruction from a single coordinate. We use ``wrong cell'' controls by injecting a cell localized to a different entity.
...
\end{equation}
For injection experiments we report a normalized score $\mathrm{RelProb}$: the mean $p_{\mathrm{ans}}$ under a condition divided by the mean under the corresponding entity-present prompt (so 1.0 indicates parity). Because this is a first-token metric, some gains can reflect lexical priming rather than full multi-token factual completion.
For entity-specific amnesia tests (main Finding 4) we define a normalized score based on log-probabilities, anchored by an unknown-entity baseline computed by swapping the entity name for a small set of unseen names and averaging the resulting answer log-probabilities.
...
\textbf{Result.} Localization is strongly non-uniform (Figure~\ref{fig:layerhist}): 99.0\% of entities peak in layers 0--5, and only 1.0\% peak in layers 22 or 27. Since ranking is over all 28 layers, this pattern is empirical rather than enforced. We interpret it as evidence that entity cells act as early identity-canonicalization features.
\end{finding}
...
\begin{finding}{Finding 2: Controlled Injection Provides Directional Causal Modulation}
\textbf{Experiment.} For each trustworthy PopQA-200 entity (Appendix~\ref{app:entity_neurons}), we run $K{=}2$ questions, replace one entity mention with \texttt{X}, and intervene at that token. We first set the full vector at \texttt{X} to a mean-entity initialization, then overwrite selected coordinates with either the entity's top-$k$ cells ($k{=}5$) or a wrong-entity top-$k$ control. We sweep $\alpha \in \{1,2,4,8,16,32,64,128,200\}$ per entity and report the best value.

\textbf{Result.} Over the trustworthy subset ($k=131$ entities), mean-entity initialization alone yields $\mathrm{RelProb}=0.067$, correct-cell injection increases this to 0.559, and wrong-cell injection reaches only 0.075 (Figure~\ref{fig:f4}). The no-injection baseline is nearly identical to mean-entity initialization (0.067), so we omit it from the main plot for readability. Using an entity-level success criterion ($\mathrm{RelProb}\ge 0.30$, at least +0.05 over no-injection and +0.05 over wrong-cell), 77/131 entities succeed with top-$k$ injection, compared with 75/131 under top-1 only; only 3 entities require top-$k$ to pass. This supports directional modulation on an existing entity manifold, not single-cell reconstruction of full factual content.
\end{finding}
...
\caption{Activation causality on trustworthy entities ($k=131$, $n=262$). $\mathrm{RelProb}$ is normalized by the entity-present condition. Interventions at \texttt{X} use mean-entity initialization plus top-$k$ overwrite ($k{=}5$, per-entity $\alpha$ sweep). Correct-cell injection strongly exceeds wrong-cell control.}
\label{fig:f4}
\end{figure}

\begin{finding}{Finding 3: Localization Tracks Identity Across Surface Forms}
...
\textbf{Result.} The same cell (L2-N10941) remains top-ranked across spelling and phrasing variants of ``Barack Obama'' (Figure~\ref{fig:f3}). We observe similar robustness for acronym and multilingual surface forms (Figures~\ref{fig:f3_acronym} and~\ref{fig:f3_multilingual}), supporting a canonicalization role: the localized feature tracks latent identity across tokenization changes rather than memorizing one surface token sequence.
\end{finding}
...
\caption{Multilingual robustness: ``Paris'' written in different scripts (Latin, Hebrew, Chinese, Arabic) localizes to the same top-ranked cell (L1-N231), consistent with surface-form canonicalization into a shared identity coordinate.}
\label{fig:f3_multilingual}
\end{figure}
\FloatBarrier

\begin{finding}{Finding 4: Negative Ablation Induces Entity-Specific Amnesia}
...
\textbf{Result.} Target retention drops from 1.0 to 0.430 at $\alpha=-3$, while control retention stays near baseline (1.0 to 0.999; Figure~\ref{fig:f6}). This pattern is consistent with entity-specific amnesia: ablation disrupts access to an identity signal for the targeted entity without broad behavioral collapse.
\end{finding}

\begin{figure}[!h]
\centering
\includegraphics[width=\columnwidth]{figures/f6_unlearning_obama_trump.pdf}
\caption{Entity-specific amnesia under negative ablation for the localized Obama cell (L2-N10941). Target (Barack Obama) recall drops substantially as $\alpha$ decreases, while control (Donald Trump) remains near baseline.}
\label{fig:f6}
\end{figure}

\section{Discussion}
The combined evidence supports a canonicalization-and-control view of entity cells in Qwen2.5-7B base. Localization is strong, early-layer concentrated, and robust across typos, acronyms, and multilingual forms, which is consistent with latent identity anchoring. Controlled injection then shows that these anchors are causally actionable: moving selected coordinates from a shared mean-entity state shifts answer probability in the expected direction, while wrong-cell controls do not. Negative ablation produces entity-specific amnesia, suggesting loss of access to an identity pointer rather than erasure of all related knowledge. Cross-model extension (Appendix~\ref{app:cross_model}) is mostly negative: outside Qwen2.5-7B-Instruct and OLMo-7B, the same signals are much weaker.

\section{Limitations and Scope}
This study focuses on one base model and one dataset family. The stability score is heuristic and may miss distributed or multi-cell codes. Metrics are mostly first-token based, which can understate multi-token factual competence and can reflect lexical priming effects. We use $K{=}2$ prompts per entity for localization and causal checks, which can introduce per-entity instability. In Finding 2, $\alpha$ is selected per entity from a sweep; this improves sensitivity but can introduce optimistic bias, and a fixed-$\alpha$ protocol is an important next step. Our injection and ablation experiments are still narrow in relation coverage. We also include an exploratory \emph{factual modification} procedure via latent steering: optimizing a small perturbation injected at an entity-associated activation site to rewrite a specific relation while preserving unrelated facts; Appendix~\ref{app:latent_steering} provides a concrete template (Algorithm~\ref{alg:steer}) and prompt set.

\section{Conclusion}
Entity behavior in Qwen2.5-7B base is strongly shaped by sparse, stable, and causally actionable entity cells. Across a curated PopQA-200 benchmark, we find early-layer concentration, robustness to surface-form variation, directional causal modulation under controlled injection, and entity-specific amnesia under negative ablation. These results support a practical mechanistic picture in which localized cells function as latent identity anchors within larger circuits, offering concrete intervention points for interpretable factual control.
...
\paragraph{Key hyperparameters.} Unless stated otherwise: curated PopQA $N=200$ entities, $K=2$ questions per entity for localization and causal checks, seed 7, and $\epsilon=10^{-6}$ for stability computations. Entity-specific amnesia (main Finding 4) uses $\alpha \in [1, -3]$ with 20 steps.
\paragraph{Finding 2 injection setting.} For Finding 2 we use set-injection at the placeholder position with mean-entity initialization. Concretely, we first set the full hidden vector at \texttt{X} to the layer-specific mean-entity vector, then overwrite the selected top-$k$ coordinates ($k{=}5$). We sweep an interpolation/extrapolation factor $\alpha \in \{1,2,4,8,16,32,64,128,200\}$ per entity, selecting the best-performing $\alpha$ for reporting; this choice is intentionally high-sensitivity and may be optimistic relative to a fixed-$\alpha$ protocol. We additionally flag entity-level success when $\mathrm{RelProb}\ge 0.30$ and both margins $\mathrm{RelProb}-\mathrm{RelProb}_{\mathrm{no\ inj}}\ge 0.05$ and $\mathrm{RelProb}-\mathrm{RelProb}_{\mathrm{wrong}}\ge 0.05$.
...
We rerun the same pipeline on six additional open $\sim$7B checkpoints: PopQA-200 localization ($K{=}2$), causal injection on found cells, and variant robustness checks. Coordinates are re-localized per model (no coordinate transfer). The main outcome is negative: outside Qwen2.5-7B-Instruct and OLMo-7B, the causal and robustness signals are weak.
...
\bottomrule
\end{tabular}
\caption{Cross-model summary. $\Delta$(C-W) is the correct-minus-wrong injection gap in relative answer probability. ``Succ.'' is the Finding~2 entity-level success criterion. ``Var. match'' is exact top-cell match across 10 surface-form probes.}
...
\paragraph{Takeaway.}
The findings do not transfer uniformly across model families. While sparse candidates can be localized broadly, strong causal separation and variant robustness are concentrated in Qwen2.5-7B-Instruct and OLMo-7B.

\paragraph{Why might Qwen be stronger?}
We see three plausible hypotheses. First, Qwen tokenization and pretraining coverage may better canonicalize noisy aliases, acronyms, and multilingual forms into stable early-layer features. Second, architecture and optimization details may induce sharper sparse gating in early MLP blocks, which makes top-cell interventions more effective. Third, the Qwen data mixture may produce stronger entity-centric QA behavior at next-token level, which aligns better with our metric and intervention protocol. These are hypotheses; distinguishing them requires targeted ablations.
