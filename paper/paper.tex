...
\begin{abstract}
Large language models exhibit strong entity-centric behavior, but the internal features that identify entities across surface forms remain poorly characterized. We present a mechanistic study in Qwen2.5-7B base that localizes sparse entity-sensitive MLP features (entity cells), validates them with causal interventions, and tests whether modulation can remain selective. Across a curated PopQA-200 benchmark, we find stable entity cells concentrated in early layers and robust to typos, acronyms, and multilingual forms. Negative ablation induces entity-specific amnesia with minimal control-entity drift and provides a scalable trust filter for localized cells; on that trustworthy subset, controlled injection on top of a mean-entity initialization increases answer-token probability for the matched entity relative to wrong-cell controls, with mostly single-cell sufficiency. Extending the same pipeline to six additional open $\sim$7B models shows limited transfer: sparse candidates are detectable, but strong causal and robustness signals are concentrated in the Qwen family (and partly OLMo). These results support a practical view of entity cells as canonicalization anchors and pointer-like control features for interpretable factual modulation.
\end{abstract}
...
\paragraph{Overview of Findings.} The four primary findings establish early-layer concentration, robustness to name variants, entity-specific amnesia under negative ablation, and causal modulation under controlled injection. Appendices provide prompt templates, algorithms, and the full PopQA-200 entity cell map (neuron coordinates).
...
For entity-specific amnesia tests (main Finding 3) we define a normalized score based on log-probabilities, anchored by an unknown-entity baseline computed by swapping the entity name for a small set of unseen names and averaging the resulting answer log-probabilities.
...
\caption{Layer of the top localized cell for each PopQA-200 entity (Qwen2.5-7B base, $n=200$).}
\label{fig:layerhist}
\end{figure}

\begin{finding}{Finding 2: Localization Tracks Identity Across Surface Forms}
\textbf{Experiment.} We repeat localization under surface-form perturbations while holding relation prompts fixed. We test (i) spelling/phrasing variants (Barack Obama), (ii) acronym variants (FBI), and (iii) multilingual variants (Paris).
...
\caption{Acronym robustness (FBI): variants localize to the same top-ranked cell (L2-N11955).}
...
\caption{Multilingual robustness (Paris): variants localize to the same top-ranked cell (L1-N231).}
\label{fig:f3_multilingual}
\end{figure}
\FloatBarrier

\begin{finding}{Finding 3: Negative Ablation Induces Entity-Specific Amnesia}
\textbf{Experiment.} We apply negative ablation to localized cells and track entity-level recall under a non-collapse sanity check. We run this both as a detailed case study (Obama vs.\ Trump control) and as a dataset-scale trust screen over PopQA-200 localized cells.

\textbf{Result.} In the case study, target retention drops from 1.0 to 0.430 at $\alpha=-3$, while control retention stays near baseline (1.0 to 0.999; Figure~\ref{fig:f6}). At scale, the same ablation criterion is used to mark cells as trustworthy: 131/200 localized cells pass and define the subset used in Finding 4.
\end{finding}
...
\begin{finding}{Finding 4: Controlled Injection Shows Predominantly Single-Cell Sufficiency}
\textbf{Experiment.} On the trustworthy subset from Finding 3 (Appendix~\ref{app:entity_neurons}), we run $K{=}2$ questions per entity, replace one entity mention with \texttt{X}, and intervene at that token. We first set the full vector at \texttt{X} to a mean-entity initialization, then inject either a single top cell (top-1) or a small top-$k$ set (top-3/top-5); wrong-entity cells are used as controls. We sweep $\alpha \in \{1,2,4,8,16,32,64,128,200\}$ per entity and report the best value.

\textbf{Result.} Mean-entity initialization alone yields $\mathrm{RelProb}=0.067$, correct-cell injection increases this to 0.559, and wrong-cell injection reaches only 0.075 (Figure~\ref{fig:f4}). The key result is that single-cell injection is already sufficient in most cases: 75/131 entities pass with top-1, versus 77/131 with top-$k$; only 3 entities require multi-cell injection. Top-3/top-5 variants therefore add little over top-1 in this setting.
\end{finding}
...
\caption{Activation causality on trustworthy entities ($k=131$, $n=262$). $\mathrm{RelProb}$ is normalized by the entity-present condition. Interventions at \texttt{X} use mean-entity initialization plus cell overwrite ($k{=}1$ or $k{=}5$, per-entity $\alpha$ sweep). Correct-cell injection strongly exceeds wrong-cell control.}
\label{fig:f4}
\end{figure}

\section{Discussion}
The combined evidence supports a canonicalization-and-control view of entity cells in Qwen2.5-7B base. Localization is strong, early-layer concentrated, and robust across typos, acronyms, and multilingual forms, which is consistent with latent identity anchoring. Negative ablation shows entity-specific amnesia and provides a practical trust filter at scale (131/200). On that trustworthy subset, controlled injection is strongly directional and mostly single-cell sufficient (75/131 with top-1 vs.\ 77/131 with top-$k$). Cross-model extension (Appendix~\ref{app:cross_model}) is mostly negative: outside Qwen2.5-7B-Instruct and OLMo-7B, the same signals are much weaker.

\section{Limitations and Scope}
This study focuses on one base model and one dataset family. The stability score is heuristic and may miss distributed or multi-cell codes. Metrics are mostly first-token based, which can understate multi-token factual competence and can reflect lexical priming effects. We use $K{=}2$ prompts per entity for localization and causal checks, which can introduce per-entity instability. In Finding 4, $\alpha$ is selected per entity from a sweep; this improves sensitivity but can introduce optimistic bias, and a fixed-$\alpha$ protocol is an important next step. Our injection and ablation experiments are still narrow in relation coverage. We also include an exploratory \emph{factual modification} procedure via latent steering: optimizing a small perturbation injected at an entity-associated activation site to rewrite a specific relation while preserving unrelated facts; Appendix~\ref{app:latent_steering} provides a concrete template (Algorithm~\ref{alg:steer}) and prompt set.

\section{Conclusion}
Entity behavior in Qwen2.5-7B base is strongly shaped by sparse, stable, and causally actionable entity cells. Across a curated PopQA-200 benchmark, we find early-layer concentration, robustness to surface-form variation, entity-specific amnesia under negative ablation, and directional controlled injection that is mostly single-cell sufficient. These results support a practical mechanistic picture in which localized cells function as latent identity anchors within larger circuits, offering concrete intervention points for interpretable factual control.
...
\paragraph{Key hyperparameters.} Unless stated otherwise: curated PopQA $N=200$ entities, $K=2$ questions per entity for localization and causal checks, seed 7, and $\epsilon=10^{-6}$ for stability computations. Entity-specific amnesia (main Finding 3) uses $\alpha \in [1, -3]$ with 20 steps.
\paragraph{Finding 4 injection setting.} For Finding 4 we use set-injection at the placeholder position with mean-entity initialization. Concretely, we first set the full hidden vector at \texttt{X} to the layer-specific mean-entity vector, then overwrite the selected top-$k$ coordinates (single-cell top-1 as primary, with small top-$k$ variants such as top-3/top-5). We sweep an interpolation/extrapolation factor $\alpha \in \{1,2,4,8,16,32,64,128,200\}$ per entity, selecting the best-performing $\alpha$ for reporting; this choice is intentionally high-sensitivity and may be optimistic relative to a fixed-$\alpha$ protocol. We additionally flag entity-level success when $\mathrm{RelProb}\ge 0.30$ and both margins $\mathrm{RelProb}-\mathrm{RelProb}_{\mathrm{no\ inj}}\ge 0.05$ and $\mathrm{RelProb}-\mathrm{RelProb}_{\mathrm{wrong}}\ge 0.05$.
...
The full PopQA-200 entity map is grouped by category to improve readability. The current split contains 48 people, 82 locations, 6 organizations, and 64 other entities. Each row includes a trust flag computed by automated checks (e.g., early-layer localization and causal sensitivity under negative ablation, plus non-collapse sanity checks). Under these checks, $k=131$ out of $n=200$ localized cells are marked trustworthy. Under the Finding 4 causal-injection success criterion, 77/131 trustworthy entities pass with top-$k$ injection (75/131 with top-1); 3 entities require top-$k$.
...
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{1.08}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.32\columnwidth}rrrr@{}}
\toprule
\textbf{Model} & \textbf{Med. L} & \textbf{$\Delta$(C-W)} & \textbf{Succ.} & \textbf{Var.} \\
\midrule
Qwen2.5-7B-Inst & 3.0 & 0.472 & 27/40 & 10/10 \\
OLMo-7B & 2.0 & 0.544 & 23/37 & 3/10 \\
Llama-3.1-8B-Inst & 11.0 & 0.016 & 5/40 & 5/10 \\
Gemma-2-9B-it & 7.5 & 0.001 & 4/40 & 5/10 \\
Mistral-7B-v0.3 & 13.0 & 0.006 & 5/40 & 4/10 \\
OpenLLaMA-7B & 10.0 & 0.116 & 12/33 & 3/10 \\
\bottomrule
\end{tabular}
\caption{Cross-model summary (all $n{=}200$). $\Delta$(C-W) is the correct-minus-wrong injection gap in relative answer probability. ``Succ.'' is the Finding~4 entity-level success criterion. ``Var.'' is exact top-cell match across 10 surface-form probes.}
