...
We study Qwen2.5-7B base \cite{qwen25}. Working with a base model is deliberate: unlike instruction-tuned models, base models do not reliably follow QA formats, so causal effects must be measured carefully with prompt styles and metrics that reflect next-token prediction. We additionally report cross-model localization results in Appendix~\ref{app:cross_model}, including Qwen2.5-7B-Instruct and other open $\sim$7B checkpoints.
...
\caption{PopQA-style activation causality on trustworthy entity cells ($k=131$ entities out of $N=200$; $K{=}2$ questions per entity, $n=262$ examples). Bars report $\mathrm{RelProb}$ (answer-token probability divided by the entity-present probability); the entity-present condition is 1.0 by construction and shown explicitly. Interventions inject top-$k$ cells ($k{=}5$) at the placeholder position after mean-entity initialization, with per-entity $\alpha$ selected by sweeping $\{1,2,4,8,16,32,64,128,200\}$. Error bars are bootstrap standard deviations. Correct-cell injection increases $\mathrm{RelProb}$ relative to both no-injection and wrong-cell controls, and yields 77/131 successful entities under the criterion in Finding 2 (75/131 with top-1 only; 3 top-$k$-needed cases).}
\label{fig:f4}
\end{figure}
...
\section{Discussion}
The combined evidence supports a sparse-key view of entity knowledge in Qwen2.5-7B base. Localization is strong, concentrated in early blocks, and robust to surface-form variation. Controlled injection recovers answer-token probability in QA-style prompts relative to no-injection and wrong-cell controls, and negative ablation can selectively degrade entity knowledge relative to a control entity without collapsing model behavior. In cross-model localization runs (Appendix~\ref{app:cross_model}), strong top-cell dominance persists across additional open $\sim$7B models, while layer depth concentration varies by architecture, suggesting transfer with non-trivial representation shifts.
...
\paragraph{Finding 2 injection setting.} For Finding 2 we use set-injection at the placeholder position with mean-entity initialization. We inject the top-$k$ cells within the localized layer ($k{=}5$) and sweep an interpolation/extrapolation factor $\alpha \in \{1,2,4,8,16,32,64,128,200\}$ per entity, selecting the best-performing $\alpha$ for reporting. We additionally flag entity-level success when $\mathrm{RelProb}\ge 0.30$ and both margins $\mathrm{RelProb}-\mathrm{RelProb}_{\mathrm{no\ inj}}\ge 0.05$ and $\mathrm{RelProb}-\mathrm{RelProb}_{\mathrm{wrong}}\ge 0.05$.

\section{Extension to Other Models}
\label{app:cross_model}
We run the full localization pipeline (Section 4.1) on six additional open $\sim$7B checkpoints, using the same curated PopQA-200 entity list and $K{=}2$ questions per entity. For each model, we recompute baseline normalization, localize one top cell per entity over all layers, and report distributional statistics and sample coordinates. This tests whether the \emph{localization signal} itself extends beyond Qwen2.5-7B base. Full model-specific PopQA-200 coordinate maps are included in the accompanying artifacts.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{$n$} & \textbf{\% L$\le 6$} & \textbf{\% Dom$\ge 10$} & \textbf{Med. L} \\
\midrule
Qwen2.5-7B & 200 & 99.0 & 87.5 & 2.0 \\
Qwen2.5-7B-Inst & 200 & 69.5 & 78.0 & 3.0 \\
OLMo-7B & 200 & 73.0 & 63.5 & 2.0 \\
Gemma-2-9B-it & 200 & 48.5 & 78.0 & 7.5 \\
OpenLLaMA-7B & 200 & 43.0 & 73.0 & 10.0 \\
Llama-3.1-8B-Inst & 200 & 33.5 & 83.5 & 11.0 \\
Mistral-7B-v0.3 & 200 & 36.0 & 81.5 & 13.0 \\
\bottomrule
\end{tabular}
\caption{Cross-model localization summary on the same PopQA-200 entity set ($K{=}2$ questions per entity). ``dom.'' denotes top-cell dominance (top-1 stability divided by mean of the next top-5 in the same layer). High dominance is common across models, while early-layer concentration varies substantially.}
...
\midrule
Qwen2.5-7B & L2-N10941 & L1-N11948 & L1-N231 & L1-N18670 \\
...
\bottomrule
\end{tabular}
\caption{Localized entity-cell coordinates for representative entities across models (same localization pipeline and PopQA-200 entity list). Coordinates vary across checkpoints, but each model yields identifiable high-dominance cells.}
...
\caption{Share of localized entities peaking in layers 0--6 across models, using the same PopQA-200 localization setup. Early-layer concentration is strongest in Qwen2.5-7B base and remains substantial in Qwen2.5-7B-Instruct and OLMo-7B, but is weaker in Mistral, Llama-3.1, Gemma-2, and OpenLLaMA.}
